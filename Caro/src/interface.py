...
class Menu:
    def __init__(self, root):
        ...
        self._create_button(text=options[0], command=self.pvp)
        self._create_button(text=options[1], command=self.pvc)
        self._create_button(text=options[2], command=self.load_game)
        self._create_button(text=options[3], command=quit)
        self.canvas.pack(padx=10, pady=10)
        Frame(master=self.canvas).pack()
        
    def _create_button(self, text="Not set", x=0, y=0, width=BTN.width, height=BTN.height, command=None):
        btn = Button(self.canvas, width=width,height=height, text=text,font="Impact 20",command=command).pack()
        return btn

    def _create_board_game(self, title="window"):
        window = BoardWidget(self._create_window(title))

    def _create_popup(self, master, title=""):
    ...
    def pvp(self):
        ...
        pop_window = self._create_popup(self.canvas)
